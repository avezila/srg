...
class OfferShort extends Component {
  ...
  onClickAddToFavorite = () =>
    this.props.changeFavorite({
      favoriteIDs: {
        [this.props.offer.id]: !this.props.isFavorite,
      },
    })
  onClickAddToReport = () =>
  ...
  render () {
    let {offer, isFavorite, isAdded} = this.props
    ...
    if (offer.price) {
      cols.push(
        <div title='Общая цена' key='price' className={s.price}>
          {offer.price.toString()::PrettyInt() + ' руб.'}
        </div>
      )
    }
    ...
    if (offer.type === 'FLAT' || offer.type === 'COMMERCIAL') {
      ...
      if (storeys || offer.floor) {
        cols.push(
          <div title='Этаж/Всего этажей' key='floor' className={s.floor}>{`${offer.floor || ''}/${storeys || ''}`}</div>
        )
      }
    }
    ...
    if (space) {
      cols.push(
        <div title='Площадь кухни/Общая площадь' key='space' className={s.space}>
          {`${space}м`}<sup>2</sup>
        </div>
      )
    }
